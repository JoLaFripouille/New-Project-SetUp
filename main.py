...
class ProjectSetupApp(customtkinter.CTk):
    def __init__(self):
        ...
        self.project_name_label = customtkinter.CTkLabel(
            self, text="Project Name:", bg_color="#05040A", fg_color="#05040A", text_color="white", font=self.font
        )
        self.project_name_label.pack(pady=10)
        
        self.project_name_entry = customtkinter.CTkEntry(self, width=300, font=self.font)
        self.project_name_entry.pack(pady=5)
        
        # Directory selection
        self.dir_label = customtkinter.CTkLabel(
            self, text="Select Directory:", bg_color="#05040A", fg_color="#05040A", text_color="white", font=self.font
        )
        self.dir_label.pack(pady=10)
        
        self.dir_button = customtkinter.CTkButton(
            self, text="Browse", command=self.browse_directory, fg_color="#4e4e4e", text_color="white", corner_radius=10, font=self.font
        )
        self.dir_button.pack(pady=5)
        
        self.dir_path = StringVar()
        self.dir_path_label = customtkinter.CTkLabel(
            self, textvariable=self.dir_path, bg_color="#05040A", fg_color="#05040A", text_color="white", font=self.font
        )
        self.dir_path_label.pack(pady=5)
        
        # Options for folders and files
        self.options_frame = customtkinter.CTkFrame(
            self, bg_color="#05040A", fg_color="#05040A", corner_radius=10, border_width=1, border_color="#7F8AB5"
        )
        ...
        for option in self.option_vars:
            customtkinter.CTkCheckBox(
                self.options_frame, text=option, variable=self.option_vars[option], onvalue="on", offvalue="off",
                bg_color="#05040A", fg_color="#4e4e4e", text_color="white", font=self.font
            ).pack(anchor="w")
        
        # Create project button
        self.create_button = customtkinter.CTkButton(
            self, text="Create Project", command=self.create_project, fg_color="#4e4e4e", text_color="white", corner_radius=10, font=self.font
        )
    ...
    def create_project(self):
        ...
        if not project_name or not base_dir:
            messagebox.showerror("Error", "Please specify a project name and directory.")
            return
...
""")
        ...
        code_path = r"C:\Users\<YourUserName>\AppData\Local\Programs\Microsoft VS Code\bin\code.exe"
